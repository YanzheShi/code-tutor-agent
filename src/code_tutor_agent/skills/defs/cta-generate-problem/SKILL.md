...
> **知识点含义说明（非常重要）**：当 {topic} 为中文「图」时，指的是**图论（graph theory，由顶点 vertex 与边 edge 构成的数据结构及其算法，例如 DFS/BFS 遍历、连通分量、最短路径、拓扑排序）**，**绝不是图片 / 图像 / 像素**，也**不是二维网格矩阵 grid（如「岛屿周长 Island Perimeter」那类用 `grid: List[List[int]]` 表示的题）**。「图」与「网格」不可混淆。同理，「树」指二叉树 / 树结构（tree），不是「树林照片」。请严格按 {topic} 的**真实算法含义**出题。
