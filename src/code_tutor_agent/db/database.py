...
def _init_db_tables(cursor) -> None:
    ...
    # ── 邮箱验证码（忘记密码 + 注册双确认共用， Brevo 可选；存哈希不存明文）──
    cursor.execute("""
    ...
    """)
    # 兼容存量库：purpose 列以 ALTER 追加（幂等）。'reset'=忘记密码；'register'=注册验证。
    cursor.execute(
        "ALTER TABLE password_reset_codes ADD COLUMN IF NOT EXISTS purpose TEXT NOT NULL DEFAULT 'reset'"
    )
    ...
    """)
    # 用户反馈（2026-09-20，docs/feedback-feature-plan.md）
    # 口径：登录用户提交，管理端**只读**（无 status 流转 / 无处理备注，决策见 plan）。
    # user_email / user_agent 为提交时快照 —— 用户注销后反馈仍可追溯。
    cursor.execute("""
    ...
    """)
    # 列表查询是 WHERE category = ? ORDER BY id DESC LIMIT/OFFSET，
    # (category, id) 复合索引可反向扫满足；不另建 created_at 单列索引（无时间范围查询，避免死索引）。
    cursor.execute(
        "CREATE INDEX IF NOT EXISTS idx_feedback_category_id "
        "ON feedback(category, id DESC)"
    )
...
# ── 用户反馈（2026-09-20，docs/feedback-feature-plan.md）──
# 口径：登录用户（含 trial）提交，管理端**只读**列表，无状态流转 / 无处理备注。
...
def create_feedback(
    ...
    """落一条用户反馈。返回行 id；失败返回 0。

    user_email / user_agent 是**提交时快照**（本项目其他表只存 user_id，此处刻意破例）：
    用户注销或升级浏览器后，反馈仍能追溯到「谁在什么环境下提的」。
    可选字段传空串时统一存 NULL，避免管理端出现 '' 与 NULL 两种空值形态。
    """
    try:
        def _do(cursor):
            cursor.execute(
                "INSERT INTO feedback "
                "(user_id, user_email, category, content, contact, screen, "
                " problem_id, session_id, user_agent, created_at) "
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, LOCALTIMESTAMP::text) RETURNING id",
                (
                    int(user_id), user_email or "", category, content,
                    contact or None, screen or None,
                    problem_id, session_id or None, user_agent or None,
                ),
            )
            return int(cursor.lastrowid or 0)
        return _with_conn(_do)
    except Exception as exc:
        logger.warning("create_feedback failed (non-fatal): %s", exc)
        return 0
...
def list_feedback(
    ...
    """管理端反馈列表（只读），按 id 倒序。category=None 表示全部分类。失败返回 []。"""
    try:
        def _do(cursor):
            sql = (
                "SELECT id, user_id, user_email, category, content, contact, "
                "screen, problem_id, session_id, user_agent, created_at "
                "FROM feedback"
            )
            params: list[object] = []
            if category:
        ...
        logger.warning("list_feedback failed (non-fatal): %s", exc)
        return []


def count_feedback(category: str | None = None) -> int:
    """反馈条数（可按 category 过滤），供分页算总页 / 前端「共 n 条」。失败返回 0。"""
    try:
        def _do(cursor):
            if category:
        ...
        logger.warning("count_feedback failed (non-fatal): %s", exc)
        return 0


def count_feedback_by_category() -> dict[str, int]:
    """各分类反馈数（管理端筛选 tab 角标，一次请求拿全）。只返回有数据的分类；失败返回 {}。"""
    try:
        def _do(cursor):
        ...
        logger.warning("count_feedback_by_category failed (non-fatal): %s", exc)
        return {}
...
def claim_trial_user(user_id: int, email: str, password_hash: str) -> bool:
    """测试用户原地转正：占位邮箱 → 真实邮箱、role='test' → 'user'，id 不变。

    仅命中 role='test' 的行（防重复转正 / 普通用户被误改）。邮箱 UNIQUE 冲突由调用方转 409。
    """
    try:
        def _do(cursor):
            cursor.execute(
                "UPDATE users SET email = ?, password_hash = ?, role = 'user' "
                "WHERE id = ? AND role = 'test'",
                (email, password_hash, user_id),
            )
            return cursor.rowcount > 0
        ok = _with_conn(_do)
        logger.info("claim_trial_user(%s → %s) → %s", user_id, email, ok)
        return ok
    except Exception as exc:
        logger.error("claim_trial_user(%s, %s) failed: %s", user_id, email, exc)
        raise
...
# ── 邮箱验证码（忘记密码 'reset' + 注册 'register' 共用，purpose 隔离互不混用）──

def save_password_reset_code(email: str, code_hash: str, expires_at: str,
                             purpose: str = "reset") -> None:
    """落一条验证码（同一邮箱+purpose 旧码作废，防多码并存混乱）。"""
    def _do(cursor):
        cursor.execute(
            "UPDATE password_reset_codes SET used = 1 WHERE email = ? AND purpose = ?",
            (email, purpose),
        )
        cursor.execute(
            "INSERT INTO password_reset_codes (email, code_hash, expires_at, purpose) "
            "VALUES (?, ?, ?, ?)",
    ...
    try:
        ...
        logger.error("save_password_reset_code(%s, purpose=%s) failed: %s", email, purpose, exc)


def verify_password_reset_code(email: str, code_hash: str, purpose: str = "reset") -> bool:
    """校验验证码：存在、未用、未过期、哈希匹配、purpose 一致。"""
    try:
        row = _with_conn(lambda cursor: cursor.execute(
            "SELECT 1 FROM password_reset_codes "
            "WHERE email = ? AND code_hash = ? AND used = 0 AND purpose = ? "
            "AND expires_at > LOCALTIMESTAMP LIMIT 1",
            (email, code_hash, purpose),
        ).fetchone())
        return row is not None
    except Exception as exc:
        logger.error("verify_password_reset_code(%s, purpose=%s) failed: %s", email, purpose, exc)
        return False


def consume_password_reset_code(email: str, code_hash: str, purpose: str = "reset") -> None:
    """用后作废。"""
    try:
        _with_conn(lambda cursor: cursor.execute(
        ...
        logger.error("consume_password_reset_code(%s, purpose=%s) failed: %s", email, purpose, exc)


def invalidate_password_reset_code(email: str, purpose: str = "reset") -> None:
    """按邮箱+purpose 作废当前验证码（不论哈希）——验证码错误次数超限时调用（审计 F-09）。"""
    try:
        _with_conn(lambda cursor: cursor.execute(
            "UPDATE password_reset_codes SET used = 1 WHERE email = ? AND used = 0 AND purpose = ?",
            (email, purpose),
        ))
    except Exception as exc:
        logger.error("invalidate_password_reset_code(%s, purpose=%s) failed: %s", email, purpose, exc)
