...
## 题型限制（重要）
- **只出「单方法函数题」**：`class Solution` 中只有一个需要实现的方法，传入输入、一次调用返回结果即完成判定。
- **禁止「设计类 / 交互类题目」**：不要出需要实现带多个方法的类、并按操作序列多次调用的题目，例如 LRU 缓存、最小栈、实现队列 / 前缀树 / 迭代器等（判题引擎暂不支持，出了也会被系统拒绝）。
- **主类必须是 Solution**：系统会硬校验 starter_code 与 optimal_solution 的主类名，凡主类不是 `Solution`（如 `class LRUCache`）的题目一律判不合格并重试。
- 若知识点本身偏数据结构设计（如栈、队列、哈希表），请改为考察其**应用**的一次性算法题，例如「用栈判定括号序列是否合法」「用哈希表统计出现次数最多的元素」。
