...
def agent_dialog_node(state: SessionState) -> Command:
    ...
    if not state.agent_dialog_history:
        ...
        # Re-enter dialog: preserve existing history without duplication
        hist = list(state.agent_dialog_history)
        tut = list(state.tutor_messages)
        logger.info("Re-enter dialog — preserved existing history")
