...
def _handle_adversarial_fail(state: SessionState) -> Command:
    ...
    logger.debug("Returning Command with goto=%s", 'return Command(update=update, goto="wait_for_submit_node")')
...
def _handle_ac(state: SessionState) -> Command:
    ...
    logger.debug("Returning Command with goto=%s", 'return Command(update=update, goto="planner_node")')
