...
![Status](https://img.shields.io/badge/status-Production%20Live-2ea043)

> 🌐 **生产环境已上线**：<https://codetutor.fun/> —— 多用户 SaaS，持续迭代中。
...
- **护栏节点（Critic）负责换题路由与 episode 收尾**：在一题终结（真实提交 AC / 换题 abandon）时 flush `problem_history`

除核心教学闭环外，项目已是一套**生产级落地系统**（🟢 线上运行：<https://codetutor.fun/>），具备完整的企业级运行基座：

- **多用户隔离**：JWT（PyJWT HS256）+ PBKDF2-HMAC-SHA256 密码哈希；画像 / 提交 / 成本 / 设置均按 `user_id` 隔离，题库全局共享；角色 `user / admin / test`，权限即时生效（每次请求回库校验）。
- **注册与邮箱验证**：邀请码 + 邮箱验证码注册（Brevo 邮件通道，未配置自动降级），发码限频（per-IP 1/min + per-email 5/24h）；忘记密码邮件验证码重置；免注册体验账号可**原地转正**（记录全保留）。
- **业务配额 / 流量控制**：做题 / 提问 / 判题按用户滑动窗口计量，体验账号叠加 IP 锚点防滥用；触额返回 429 + 转化钩子引导注册（自带 key 用户仅豁免 LLM 类配额，判题限频不豁免）。
- **用户级自定义 LLM（APP Key）**：用户自带 API key / base URL / model，按用户隔离、`security/secret_crypto.py` 经 Fernet 加密落库（`enc::` 前缀，明文永不回传），`CTA_ALLOW_CUSTOM_LLM` 总开关控制是否放开。
- **监控告警**：指标采集 → 13 条规则评估（rate / streak）→ 邮件通知 + 主页公告横幅 + 前端错误上报；监控层故障不影响业务主链路。
- **统一持久化与部署**：PostgreSQL 业务库 + LangGraph checkpointer 统一落库，Docker 一键编排（app-db + backend + frontend + 可选 Judge0）。

详细设计见[核心设计决策](#核心设计决策)与[技术栈](#技术栈)。
...
| 做题主页面 | 导师渐进式引导                 |
| --- |-------------------------|
| ![做题主页面](demo/做题主页面.png) | ![导师引导](demo/导师引导1.png) |
...
> 🌐 项目已上线生产环境（<https://codetutor.fun/>），核心闭环（出题 → 判题 → 辅导 → 轨迹分析 → 规划出题）可端到端跑通，并持续迭代中。API / State schema / 节点拓扑仍可能演进。
